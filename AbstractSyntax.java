...
// 각 Node를 Class로 정의하며 생성자들을 정의하여 상위 노드는 하위 노드를 갖도록 정의
// 각 Node에 display 함수를 만들어서 전체 hierarchy 구조를 표현하도록 함
import java.util.*;
...
class Program {
    ...
    // display 함수들에 인자를 주어 그 횟수만큼 tab을 하여 hierarchy 구조를 표현
...
class Declarations extends ArrayList<Declaration> {
	...
	public void display(int j){
		...
		for(int i = 0; i < size(); i++) get(i).display(j);
		System.out.println("}");
	}
}
...
// Declaration = Variable v; Type t
    ...
    Declaration(Variable var, Type type, Type in){
    	v = var; t = type; inner_T = in;
    }
    public void display(int j){
    	System.out.print(" <" + v + ", ");
    	System.out.print(t + "> ");
...
class Type {
    ...
    final static Type STACK = new Type("stack");
    final static Type QUEUE = new Type("queue");
    final static Type LIST = new Type("list");
    final static Type DEQUE = new Type("deqeue");
...
abstract class Statement {
	...
	// abstract class 이므로 이곳에 display 함수를 생성해둠
	public void display(int j){}
}
...
class Block extends Statement {
    ...
    public void display(int j){
    	...
    	for(int i = 0; i < members.size(); i++) members.get(i).display(j);
    }
}
...
// Conditional = Expression test; Statement thenbranch, elsebranch
    ...
    public void display(int j){
    	for(int i = 0; i < j; ++i) System.out.print("\t");
    	test.display(++j);
    	thenbranch.display(j);
    	elsebranch.display(j);
    }
}
...
class Push_Back extends Statement{
	Variable target;
	Expression e1;
	Push_Back(Variable v, Expression e2){
		target = v;  e1 = e2;
	}
}

class Push_Front extends Statement{
	Variable target;
	Expression e1;
	Push_Front(Variable v, Expression e2){target = v; e1 = e2;}
}

class Pop_Back extends Statement{
	Variable target;

	Pop_Back(Variable v){target = v;}

}

class Pop_Front extends Statement{
	Variable target;

	Pop_Front(Variable v){target = v;}	
}
...
class Operator {
    ...
    public void display(int j){ System.out.println(val);}
